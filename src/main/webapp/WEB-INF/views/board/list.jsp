		...
		<div class="row">
			...
			<div class="col-lg-8">
				...
				<table class="table table-bordered table-hover">
						<thead>
							<tr style="background-color: #00008B; margin-top: 0; height: 40px; color: white; border: 0px solid #f78f24; opacity: 0.8">
								<th>#번호</th>
								<th>제목</th>
								<th>작성자</th>
								<th>작성일</th>
					...
					<c:if test="${articles.size() > 0}">
						<c:forEach var="b" items="${articles}" >
							<tr style="color: black;">
								<td>${b.boardNo}</td>
								<td>
									<a style="margin-top: 0; height: 40px; color: #00ced1;" href="<c:url value='/board/content/${b.boardNo}${param.page == null ? pc.makeURI(1) : pc.makeURI(param.page)}' />">
										${b.title}
									</a>
									&nbsp;
									<c:if test="${b.newMark}">
				...
				<ul class="pagination justify-content-center">
                       ...
                       <c:if test="${pc.prev}">
						...
						<c:forEach var="pageNum" begin="${pc.beginPage}" end="${pc.endPage}">
							<li class="page-item">
							   <a href="<c:url value='/board/list${pc.makeURI(pageNum)}'/>" class=" page-link ${(pc.paging.page == pageNum) ? 'page-active' : 'page-deactive'}" style="margin-top: 0; height: 40px; border: 1px solid #00008B;">${pageNum}</a>
							</li>
					    </c:forEach>
